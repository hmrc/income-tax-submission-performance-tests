...
object AuthLoginRequests extends ServicesConfiguration {
  ...
  def postAgentLoginPage(nino: String): HttpRequestBuilder = http("Post Agent Login page")
    .post(authLoginUrl + "/auth-login-stub/gg-sign-in": String)
    .formParam("authorityId", "")
    .formParam("gatewayToken", "")
    .formParam("redirectionUrl", s"$serviceUrl/test-only/$taxYear/additional-parameters?ClientNino=$nino&ClientMTDID=1234567891")
    ...
    .formParam("enrolment[0].name", "HMRC-MTD-IT")
    .formParam("enrolment[0].taxIdentifier[0].name", "MTDITID")
    .formParam("enrolment[0].taxIdentifier[0].value", "1234567891")
    .formParam("enrolment[0].state", "Activated")
