...
import uk.gov.hmrc.perftests.itsass.requests.SelfEmploymentRequests._


trait SelfEmploymentSimSteps extends PerformanceTestRunner{

  def selfEmploymentIndividualEOYYear(id: String, description: String): JourneyPart = setup(id, description) withRequests(
    getLoginPage,
  ...
  def selfEmploymentAgentEOYYear(id: String, description: String): JourneyPart = setup(id, description) withRequests(
    getLoginPage,
    postIndividualLoginPage("AA000001C", "1234567890", taxYearEOY),
    getStartPage(taxYearEOY),
    getEOYOverviewPage(taxYearEOY),
