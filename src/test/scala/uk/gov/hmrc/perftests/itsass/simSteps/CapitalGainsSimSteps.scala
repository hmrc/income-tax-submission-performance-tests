...
import uk.gov.hmrc.perftests.itsass.requests.CapitalGainsRequests._
...
trait CapitalGainsSimSteps extends PerformanceTestRunner{

  def capitalGainsIndividual (id: String, description: String): JourneyPart = setup(id, description) withRequests (
    getLoginPage,
    postIndividualLoginPage("AA123459A", "1234567890"),
    getStartPage(taxYear),
    getCapitalGains,
    postCapitalGains,
    getChangeCapitalGains,
    postChangeCapitalGains,
    getTrustsSettlementsEstates,
    postTrustsSettlementsEstates,
    getChangeTrustsSettlementsEstates,
    postChangeTrustsSettlementsEstates
  )

  def capitalGainsAgent(id: String, description: String): JourneyPart = setup(id, description) withRequests(
    getLoginPage,
