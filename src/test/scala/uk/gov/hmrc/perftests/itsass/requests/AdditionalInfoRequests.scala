...
object AdditionalInfoRequests extends ServicesConfiguration {
  ...
  def getCheckAnswersNonDeductibleLoanInterestReliefPage(taxYear: String): HttpRequestBuilder = http("Get Check Answers Non-Deductible Loan Interest Relief Page")
    .get(s"${businessTaxReliefsUrl(taxYear)}/non-deductible-loan-interest/check-answers")
    .check(saveCsrfToken())
    .check(status.is(200))
